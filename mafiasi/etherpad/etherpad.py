...
class Etherpad(object):
    ...
    def create_group_pad(self, group_name, pad_name):
        ...
        """
        try:
            self.api.createGroupPad(groupID=self.get_group_id(group_name), padName=pad_name.encode("utf-8"))
    ...
    def create_session(self, user, group_name):
        ...
        self.api.createSession(groupID=group, authorID=user_ep, validUntil=str(time.time() + SESSION_DURATION))

    def get_session_cookie(self, user):
        sessions = self.api.listSessionsOfAuthor(authorID=self._get_ep_user(user))
    ...
    def get_group_id(self, group_name):
        return self.api.createGroupIfNotExistsFor(groupMapper=group_name.encode("utf-8"))["groupID"]
