...
def get_group_pads(group_names):
    ...
    with conn.cursor() as c:
        c.execute("SELECT key, value FROM store WHERE key IN %s", (mapper_keys,))
    ...
    with conn.cursor() as c:
        c.execute("SELECT key, value FROM store WHERE key IN %s", (tuple("group:" + group_id for group_id in groups),))
    ...
    with conn.cursor() as c:
        c.execute("SELECT key, value FROM store WHERE key IN %s", (tuple(pad_keys),))
    ...
    if rev_keys:
        with conn.cursor() as c:
            c.execute("SELECT key, value FROM store WHERE key IN %s", (tuple(rev_keys),))
