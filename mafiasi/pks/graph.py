...
def generate_graph(filenames, filter_invalid=True, restrict_keys=None, prog="neato"):
    ctx = gpgme.Context()
    ...
    for key in key_list:
        ...
        labels[keyid] = '<{0}<BR/><FONT POINT-SIZE="10">{1}</FONT>>'.format(escape_html(name), keyid)

        not_connected = signature_stats["sigcount"][keyid] == 0 and signature_stats["signedbycount"][keyid] == 0
        if not_connected:
    ...
    flipped_edges = set()
    g = pygraphviz.AGraph(directed=True, overlap="scale", splines=True, nodesep=0.2, outputMode="edgesfirst")
    ...
    for signed_keyid, signer_keyids in list(graph_dict.items()):
        for signer_keyid in signer_keyids:
            ...
            attrs = {"id": edge_id, "dir": direction}
...
def _build_signature_stats(graph_dict):
    stats = {"maxsignedbycount": 0, "maxsigcount": 0, "maxratio": 0.0, "signedbycount": {}, "sigcount": {}}
    ...
    ratios = [signedbycount[keyid] / sigcount[keyid] for keyid in graph_dict if sigcount[keyid] != 0]
    if ratios:
...
def _get_color(signature_stats, keyid):
    ...
    if sigcount[keyid] and maxratio != 0:
        green = (signedbycount[keyid] / sigcount[keyid] / maxratio * 0.75) * 2 / 3 + 1 / 3
