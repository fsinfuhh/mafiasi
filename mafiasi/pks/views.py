...
@login_required
def my_keys(request):
    keys = [key.get_keyobj() for key in AssignedKey.objects.filter(user=request.user)]
    return TemplateResponse(request, "pks/my_keys.html", {"keys": keys})
...
def autocomplete_keys(request):
    ...
    for key in keys:
        try:
            ...
            autocomplete_data.append({"value": keyid, "label": label})
        except IndexError:
            continue
...
def assign_keyid(request):
    ...
    _key, created = AssignedKey.objects.get_or_create(fingerprint=fingerprint, user=request.user)
    if created:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            for fingerprint in form.imported_keys:
                AssignedKey.objects.get_or_create(fingerprint=fingerprint, user=request.user)
            return redirect("pks_my_keys")
    else:
        form = ImportForm()

    return TemplateResponse(request, "pks/upload_keys.html", {"form": form})
...
def unassign_keys(request):
    ...
    AssignedKey.objects.filter(fingerprint__in=fingerprints, user=request.user).delete()
    return redirect("pks_my_keys")
...
def all_keys(request):
    ...
    return TemplateResponse(request, "pks/all_keys.html", {"keys": keys})
...
def graph(request, party_pk=None):
    ...
    return TemplateResponse(request, "pks/graph.html", {"graph_name": graph_name, "party": party})
...
def search(request):
    return TemplateResponse(request, "pks/search.html", {"hkp_url": settings.HKP_URL})
...
def party_list(request):
    ...
    return TemplateResponse(request, "pks/party_list.html", {"parties": parties})
...
def party_participate(request, party_pk):
    ...
    try:
        ...
        checked_fingerprints = set(key.fingerprint for key in participant.keys.all())
    except Participant.DoesNotExist:
        checked_fingerprints = set()

    if request.method == "POST":
        fingerprints = request.POST.getlist("fingerprint")
        keys = AssignedKey.objects.filter(fingerprint__in=fingerprints, user=request.user)
        ...
        if participant:
            ...
            messages.success(request, _("Successfully submitted keys to party."))
            return redirect("pks_party_keys", party.pk)
        else:
            messages.success(request, _("Not participating in this keysigning party."))
            return redirect("pks_party_list")
    ...
    return TemplateResponse(
        request,
        "pks/party_participate.html",
        {"party": party, "keys": keys, "checked_fingerprints": checked_fingerprints},
    )
...
def party_keys(request, party_pk):
    ...
    participants_qs = party.participants.select_related().order_by("user__first_name")
    participants = []
    for participant in participants_qs:
        participants.append({"user": participant.user, "keys": [key.get_keyobj() for key in participant.keys.all()]})

    return TemplateResponse(
        request, "pks/party_keys.html", {"party": party, "participants": participants, "hkp_url": settings.HKP_URL}
    )
...
def party_missing_signatures(request, party_pk):
    ...
    return TemplateResponse(
        request,
        "pks/party_missing_signatures.html",
        {"party": party, "missing_my_sigs": missing_my_sigs, "missing_other_sigs": missing_other_sigs},
    )
...
def _fill_missing_sigs(missing_sigs, own_key, other_key):
    ...
    if other_user.pk not in missing_sigs:
        missing_sigs[other_user.pk] = {"user": other_user, "keys": []}
...
def hkp_add_key(request):
    ...
    return HttpResponse("OK. {0} keys imported.".format(result.imported), content_type="text/plain")
...
def hkp_lookup(request):
    ...
    if op not in ("get", "index"):
        return HttpResponse("Operation not implemented.", status=501, content_type="text/plain")

    if not search:
        return HttpResponseBadRequest("Please provide a search term.", content_type="text/plain")
...
def _hkp_op_get(request, search, options):
    ...
    if not resp.content:
        return HttpResponseNotFound("No such key: " + search, content_type="text/plain")
    return resp
...
def _hkp_op_index(request, search, options):
    ...
    if options == "mr":
        if not key_list:
            return HttpResponseNotFound("Search returned no keys.", content_type="text/plain")
        return _hkp_op_index_mr(key_list)
    else:
        return TemplateResponse(request, "pks/search_result.html", {"search_term": search, "key_list": key_list})
...
def _hkp_op_index_mr(key_list):
    ...
    for key in key_list:
        ...
        for uid in key.uids:
            ...
            resp.write(uid_tpl.format(uid=uid_str, created=created, expires="", flags="r" if uid.revoked else ""))
    return resp
