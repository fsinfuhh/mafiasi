...
def make_filename(obj, prefix, ext):
    return "{0}/{1}-{2}.{3}".format(prefix, hexlify(os.urandom(16)), obj.pk, ext)
...
class GProt(models.Model):
    ...
    content_pdf = models.FileField(upload_to=make_gprot_filename, null=True, blank=True)
...
class Notification(models.Model):
    ...
    def __str__(self):
        return "Notification for {0} by {1}".format(self.query_or_course_name, self.user)
...
class Reminder(models.Model):
    ...
    def __str__(self):
        return 'Reminder for {0}, "{1}" on {2}'.format(
            self.user, self.course.name if self.course else "", self.exam_date
        )
