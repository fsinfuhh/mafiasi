    ...
    def __init__(self, *args, **kwargs):
        ...
        def modify_ids(prefix, elem):
            ...
            if type(second) == list:  # (department, [(pk, name), (pk, name), ...])
                return first, [("{}-{}".format(prefix, pk), name) for pk, name in second]
            else:  # (pk, name)
        ...
        self.fields["search"].choices = [modify_examiners(e) for e in examiner_groups] + [
    ...
    def clean_search(self):
        ...
        for elem in data:
            if elem.startswith("e-"):
                examiners.append(pk_to_object(Teacher, elem[2:], _("Selected examiner does not exist")))
            elif elem.startswith("c-"):
                courses.append(pk_to_object(Course, elem[2:], _("Selected course does not exist")))
            else:
...
class GProtBasicForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["examiner"].choices = Teacher.objects.as_grouped_choices()
        self.fields["course"].choices = Course.objects.as_grouped_choices()
        self.fields["labels"].choices = Label.objects.as_choices()

    def clean_course(self):
        data = self.cleaned_data["course"][0]
        return pk_to_object(Course, data, _("Selected course does not exist"))

    def clean_examiner(self):
        data = self.cleaned_data["examiner"]
        result = []
        for elem in data:
            result.append(pk_to_object(Teacher, elem, _("Selected examiner does not exist")))
        return result


class GProtCreateForm(GProtBasicForm):
    type = forms.ChoiceField(choices=[("html", _("Write online")), ("pdf", _("Upload PDF"))])
