...
class Mailinglist(models.Model):
    ...
    def get_address(self):
        return "{}@{}".format(self.group.name.lower(), settings.MAILINGLIST_DOMAIN)

    def get_bounce_address(self):
        return "{}@bounce.{}".format(self.group.name.lower(), settings.MAILINGLIST_DOMAIN)
    ...
    def moderate(self, email_obj):
        ...
        modmail = ModeratedMail.objects.create(mailinglist=self, email_content=email_content)
        ...
        for admin in self.group.properties.admins.all():
            message = self._build_moderation_mail("admin", [admin.email], modmail, email_data, admin)
            message.connection = connection
            message.send(fail_silently=True)

        # Inform the sender about moderation
        message = self._build_moderation_mail("sender", [email_obj["From"]], modmail, email_data, None)
        message.connection = connection
        message.send(fail_silently=True)
    ...
    def send_email(self, email_obj):
        ...
        try:
            ...
            try:
                ...
                rcpttos = [user.email for user in self.group.user_set.all() if user.email not in excluded_emails]
        ...
        for recipient_email, smtp_error in list(refused.items()):
            ...
            refused_rcpt, _created = RefusedRecipient.objects.get_or_create(email=recipient_email)
    ...
    def _build_moderation_mail(self, template, recipient, modmail, email_data, user):
        subject = (
            {
                "admin": ("Email an {list} wartet auf Moderation/" "Email is awaiting moderation"),
                "sender": ("Deine Email an {list} wartet auf Genehmigung/" "Your email is awaiting approval"),
            }
            .get(template)
            .format(list=self.get_address())
        )
        template_file = "mailinglist/moderated_mail_{}.mail".format(template)
        body = render_to_string(template_file, {"mailinglist": self, "moderated_mail": modmail, "user": user})
        message = mail.EmailMessage(subject=subject, body=body, from_email=self.get_bounce_address(), to=recipient)
...
class WhitelistedAddress(models.Model):
    mailinglist = models.ForeignKey(Mailinglist, on_delete=models.CASCADE, related_name="whitelist_addresses")
