...
class OtherRegisterForm(PrimaryRegisterForm):
    domain = forms.ChoiceField(choices=[(v, v) for v in settings.REGISTER_DOMAINS if v != settings.PRIMARY_DOMAIN])
...
class AdditionalInfoForm(forms.Form):
    ...
    yeargroup = forms.ModelChoiceField(queryset=Yeargroup.objects.all(), required=False)
...
class NickChangeForm(forms.Form):
    def __init__(self, user, *args, **kwargs):
        ...
        kwargs["initial"] = {"nickname": nickname}
    ...
    def save(self):
        ...
        ldap_user.display_name = "{} ({})".format(self.cleaned_data["nickname"], self.user.username)
        ldap_user.save()
...
class EmailChangeForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if Mafiasi.objects.filter(real_email=email).count() > 0:
            raise forms.ValidationError(_("This address is already associated with an account."))

        if settings.MAILINGLIST_DOMAIN and email.endswith(settings.MAILINGLIST_DOMAIN):
            raise forms.ValidationError(_("Group addresses cannot be used for this purpose."))
        elif settings.MAILCLOAK_DOMAIN and email.endswith(settings.MAILCLOAK_DOMAIN):
            raise forms.ValidationError(_("Cloak adresses cannot be used for this purpose."))
