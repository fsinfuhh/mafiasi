...
from mafiasi.registration.forms import (
    AdditionalInfoForm,
    EmailChangeForm,
    NickChangeForm,
    OtherRegisterForm,
...
def request_account(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if domain == settings.PRIMARY_DOMAIN:
                ...
                if group_slug is None:
                    ...
                    raise AssertionError(
                        f'The yeargroup {user["gid"]} was referenced in the IRZ LDAP '
                ...
                try:
                    ...
                    if account[0].isdigit():
                        ...
                        yeargroup, _created = Yeargroup.objects.get_or_create(
            ...
            return TemplateResponse(
                request,
                "registration/request_account.html",
                {
...
def create_account(request, info_token):
    ...
    return TemplateResponse(request, "registration/create_account.html", {"form": form, "username": username})
...
def account_settings(request):
    ...
    if form == "change_pw":
        ...
        if nick_change_form.is_valid():
            ...
            messages.success(request, _("Your nickname is now {}.").format(nick_change_form.cleaned_data["nickname"]))
            return redirect("registration_account")
    elif form == "change_email":
        email_change_form = EmailChangeForm(request.user, request.POST)
        if email_change_form.is_valid():
            return _verify_email(request, email_change_form.cleaned_data["email"])
...
def _verify_email(request, email):
    ...
    return _send_mail_or_error_page(
        _("Verify this address for %s" % settings.PROJECT_NAME), email_content, email, request, email
    )
...
def _finish_account_request(request, info):
    ...
    email_content = render_to_string(
        "registration/create_email.html",
        {
...
def _send_mail_or_error_page(subject, content, address, request, email_shown):
    try:
        ...
        return TemplateResponse(
            request,
            "registration/email_error.html",
            {"unkown": unknown, "error_code": error_code, "error_msg": error_msg, "recipient": wrong_email},
        )

    return TemplateResponse(
        request,
        "registration/request_successful.html",
        {
            "email": email_shown,
        },
    )
