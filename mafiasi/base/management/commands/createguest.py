...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--no-guest", action="store_true", dest="noguest", default=False, help="Do not add .guest to username"
        )
    ...
    def handle(self, *args, **options):
        ...
        if not options["noguest"]:
            ...
            if not re.match("^[a-z][a-z0-9]*.guest$", name):
                raise CommandError("Name must be alphanumeric and start with a letter")
        else:  # noguest
            if not re.match("^[a-z0-9\.]*$", name):
                raise CommandError("Name must be alphanumeric")
        ...
        mafiasi = Mafiasi.objects.create(
            username=name,
