...
class PrivacyListData(models.Model):
    privacy_list = models.OneToOneField(
        PrivacyList, on_delete=models.CASCADE, db_column="id", related_name="data", primary_key=True
    )
...
class JabberUser(models.Model):
    ...
    def set_nickname(self, nickname):
        ...
        try:
            ...
            vcard = Vcard(username=self.username, vcard=vcard_xml, created_at=now())
        vcard.save()
...
class JabberUserMapping(models.Model):
    ...
    def __str__(self):
        return "{0} owns {1}@{2}".format(self.mafiasi_user, self.jabber_user, settings.JABBER_DOMAIN)
...
def create_jabber_account(mafiasi):
    ...
    user = JabberUser.objects.create(username=mafiasi.username, created_at=now())
